...
if [ "$filter" == 1 ]; then
	$WS_HOME/1kpscripts/filter_remove_gappyTaxa_aa_fas.sh -i $in;  
	f=$in-filtered50	
else
	f=$in
fi
...
if [ "$DT" == "FAA" ]; then
  if [ -s bestModel.$ALGNAME ]; then
    ...
    cd modelselection
    ln -s $f.phylip .
    perl $DIR/ProteinModelSelection.pl $ttt > ../bestModel.$ttt
    cd ..
    test -s bestModel.$ttt && ( tar cfj modelselection-logs.tar.bz --remove-files modelselection/ )
  fi
  if [ -s bestModel.$ttt ]; then
     model=PROTGAMMA`sed -e "s/.* //g" bestModel.$ttt`
  else
