...
while read y; do
	while read x; do
		while read h; do
			...
			cat $path/*/$DT*mask"$y"sites.mask"$x"taxa$h/raxml*/RAxML_bestTree.best.addPoly > $work_dir/$DT-mask"$y"sites.mask"$x"taxa$h/$DT-mask"$y"sites.mask"$x"taxa$h.gene_trees.trees
