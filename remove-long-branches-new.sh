...
while read x; do
	...
	if [ "$b" == "" ]; then
		echo "cp $H/$x/$DT-$x-$y/$DT-$x-$y/fasttree.tre.best.addPoly.rooted.final $H/$x/$DT-$x-$y-$med-$method-filtered-long-branch/$DT-$x-$y-$med-$method-filtered-long-branch/" >> rem-lb-$med-$method-$y.t; 
	else
