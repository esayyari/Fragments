...
if [ "$donebs" == "" ]; then
 ...
 if [ $CPUS -gt 1 ]; then
  raxmlHPC-PTHREADS -m $model -T $CPUS -n best -s $f.phylip $s -N $bestN $startingtree &> ../logs/best_std.errout.$tttt
 else
  raxmlHPC -m $model -n best -s $f.phylip $s -N $bestN $startingtree &> ../logs/best_std.errout.$tttt
 fi
fi
