...
for ft in ${f[@]}; do
		...
		for c in ${clade[@]}; do
			...
			while read x; do
				cat $x/*/fasttree*best*final >> $ft-10sites-"$at"taxa-with-filtering-removed-$c/gene_trees.tree
			...
			while read x; do
                                cat $x/*/fasttree*best*final >> $ft-10sites-"$at"taxa-without-filtering-removed-$c/gene_trees.tree
                                p=`absp $x/*/fasttree*BS-all.addPoly`
			...
			ls $workdir/../long*/*/$ft*10sites*"$at"taxa*removed* | grep -v "raxml" | grep -v "removed" | grep > tmp
                        while read x; do
                                cat $x/*/fasttree*best*final >> $ft-10sites-"$at"taxa-with-filtering-with-$c/gene_trees.tree
                                p=`absp $x/*/fasttree*BS-all.addPoly`
